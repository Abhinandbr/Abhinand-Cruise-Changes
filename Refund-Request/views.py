...
MEMBERSHIP_TIERS = [
    {
        ...
        'name': 'Bronze',
        'price': 50.00,
        'description': 'Basic membership with access to exclusive content and discounts.',
    },
    {
        'id': 2,
        'name': 'Silver',
        'price': 100.00,
        'description': 'Includes Bronze benefits plus priority customer service and more.',
    },
    {
        'id': 3,
        'name': 'Gold',
        'price': 200.00,
        'description': 'All Silver benefits plus VIP events and free upgrades.',
    },
]
...
def cancel_booking(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        #     return redirect('profile')  # Redirect to a suitable page
        ...
        # return redirect('profile')  # Redirect to the user's dashboard after cancellation
            ...
            #   Redirect the user to the refund request page
...
def refund_request(request, booking_id):
    user = request.user  # Get the currently logged-in user
     ...
     # Fetch the customer for the logged-in user
    customer = get_object_or_404(Customer, user=user)

    # Fetch the booking related to the user and booking_id
    booking = get_object_or_404(Booking, id=booking_id, customer=customer)
    ...
    if request.method == "POST" and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            # Validate and save the refund request
    ...
    context = {
        ...
        'booking_id': booking.id,  # Ensure booking_id is passed
        'total_price': booking.total_price,  # Pass total_price to the template
    }
...
def booking_history(request):
    try:
        ...
        customer = Customer.objects.get(user=request.user)
        # Fetch booking history for the customer, including cruise details
    ...
    for booking in bookings:
        # Ensure cruise information is available even if booking is disassociated
        if booking.booking and booking.booking.cruise:
            booking.cruise = booking.booking.cruise  # Use the associated booking's cruise
        elif booking.cruise:
            pass  # Keep the preserved cruise in CustomerBooking
        else:
            booking.cruise = None  # No cruise information available
